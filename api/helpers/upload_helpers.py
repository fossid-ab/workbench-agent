    ...
    def _perform_upload(self, file_path: str, headers: dict) -> None:
        ...
        try:
            ...
            if file_size > self.CHUNKED_UPLOAD_THRESHOLD:
                logger.info(f"File size exceeds threshold ({self.CHUNKED_UPLOAD_THRESHOLD} bytes). Using chunked upload...")
                ...
                logger.info(f"Using regular upload for file '{upload_basename}' (size: {file_size} bytes)")
