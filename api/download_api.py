...
class DownloadAPI(APIBase):
    ...
    def _download_report(self, report_entity: str, process_id: int):
        ...
        payload = {
            ...
            "data": {"report_entity": report_entity, "process_id": str(process_id)},
        }
    ...
    def generate_report(self, scan_code: str, report_type: str = "SPDX") -> int:
        ...
        payload = {
            "group": "reports",
            "action": "generate",
            "data": {"scan_code": scan_code, "report_type": report_type},
        }
        ...
        if response.get("status") != "1":
            ...
            raise builtins.Exception(
                f"Failed to generate report for scan '{scan_code}': {error_msg}"
            )
