...
class APIBase(ProcessWaiters, StatusCheckers):
    ...
    def _send_request(self, payload: dict, timeout: int = 1800) -> dict:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Failed to decode JSON response: {response.text[:500]}", exc_info=True
                )
                raise ApiError(
                    f"Invalid JSON received from API: {e.msg}",
    ...
    def _handle_api_errors(self, parsed_json: dict, payload: dict, error_msg: str):
        ...
        # Scan-specific errors
            ...
            if is_existence_check and (
                "row_not_found" in error_msg or "Scan not found" in error_msg
            ):
                raise ScanNotFoundError(f"Scan not found")
            elif is_create_action and (
                "Scan code already exists" in error_msg
                or "Legacy.controller.scans.code_already_exists" in error_msg
            ):
