...
from ...exceptions import ValidationError, FileSystemError, ProcessError
...
def handle_blind_scan(workbench: WorkbenchAPI, params: argparse.Namespace) -> bool:
    ...
    """
    logger.info("--- Starting BLIND SCAN Command ---")
    ...
    # Determine if dependency analysis should be included in hash generation
    include_da_in_hash = getattr(params, 'run_dependency_analysis', False) or getattr(params, 'run_only_dependency_analysis', False)
    
    # Perform blind scan to generate hashes
    ...
    try:
        # Upload the generated hash file
        upload_scan_content(
            workbench=workbench,
        ...
        if run_da and not include_da_in_hash:
        ...
        logger.info(f"Blind scan command completed successfully. Found {len(results)} license entries.")
        return True
        
    finally:
        # Always cleanup the temporary hash file
        cleanup_success = cleanup_temp_file(hash_file_path)
        if not cleanup_success:
            logger.warning(f"Failed to cleanup temporary hash file: {hash_file_path}")
            logger.warning("You may need to manually remove this file.")
